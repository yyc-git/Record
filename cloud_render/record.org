...
job return Stream;
no loop, use update instead;
jobs has no own data! get/set data to common data region in DO;
...
add job manager, gameObject + transform,link component, empty pass jobs(just render all pixels to blue)
